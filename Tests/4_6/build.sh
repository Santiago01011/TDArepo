...
# Configuration
CC=gcc
CFLAGS="-Wall -Wextra -Wno-endif-labels -std=c89 -g -O2 "
LDFLAGS="-lm"
EXECUTABLE="4_6"
BINDIR="src/bin"
...
if [ $COMPILE_STATUS -eq 0 ]; then
    ...
    # Copy src/resources into the bin directory so the executable can use the same relative paths
    if [ -d "src/resources" ]; then
        echo -e "${YELLOW}Copying src/resources -> $BINDIR/resources ...${NC}"
        rm -rf "$BINDIR/resources"
        cp -a "src/resources" "$BINDIR/"
    fi
